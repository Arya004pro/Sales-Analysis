...
def _metric_score(col: str) -> int:
    c = (col or "").lower()
    s = 0
    if any(k in c for k in ("revenue", "sales", "earning", "amount", "fare", "price", "total", "profit")):
        s += 12
    if any(k in c for k in ("qty", "quantity", "units", "count", "volume", "distance", "duration")):
        s += 8
...
def _dim_score(col: str) -> int:
    ...
    if c == "name" or c.endswith("_name"):
        s += 8
    if any(k in c for k in ("city", "state", "region", "store", "branch", "category", "platform", "type")):
        s += 7
...
def _pick_date_expr(conn, table: str, cols: list[tuple[str, str]]) -> str | None:
    ...
    for c, t in cols:
        if _looks_date_col(c) and any(x in t for x in ("BIGINT", "INT8", "LONG", "HUGEINT", "INT64")):
...
def _scan_table_findings(conn, table: str, cols: list[tuple[str, str]]) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []

    metric_cols = [c for c, t in cols if _is_numeric(t) and not c.lower().endswith("_id") and not _looks_date_col(c)]
    metric_cols = sorted(metric_cols, key=lambda c: (_metric_score(c), -len(c)), reverse=True)[:5]
    if not metric_cols:
        return findings

    cat_candidates = [c for c, t in cols if _is_text(t) and not c.lower().endswith("_id")]
    cat_candidates = sorted(cat_candidates, key=lambda c: (_dim_score(c), -len(c)), reverse=True)[:8]
    cat_cols: list[str] = []
    for col in cat_candidates:
        try:
            distinct_cnt = int(conn.execute(f'SELECT COUNT(DISTINCT "{col}") FROM "{table}"').fetchone()[0] or 0)
        except Exception:
            continue
    ...
    for dim in cat_cols:
        for metric in metric_cols:
            ...
            ratio = _safe_ratio(float(top[2] or 0.0), max(0.0001, float(bottom[2] or 0.0)))
            if ratio and ratio >= 1.8 and float(top[2] or 0.0) > 0 and int(top[1] or 0) >= 12 and int(bottom[1] or 0) >= 12:
                findings.append({
                    "score": min(100.0, (ratio - 1.0) * 24.0 + min(15.0, len(rows))),
                    "type": "dimension_gap",
                    "table": table,
                    "dimension": dim,
                    "metric": metric,
    ...
    # Cancellation/refund/status rate gap discovery.
    ...
    if date_expr:
        for metric in metric_cols[:3]:
            ...
            if len(month_rows) < 4:
                continue
            month_vals = {int(r[0]): float(r[1] or 0.0) for r in month_rows if r and r[0] is not None}
            if not month_vals:
                continue
    ...
    # Per-distance earnings anomalies where possible.
    dist_cols = [c for c in metric_cols if any(k in c.lower() for k in ("distance", "km", "mile"))]
    value_cols = [c for c in metric_cols if any(k in c.lower() for k in ("revenue", "sales", "earning", "fare", "amount", "total"))]
...
async def handler(request: ApiRequest[Any], ctx: FlowContext[Any]) -> ApiResponse[Any]:
    refresh = _query_param(request, "refresh", "").strip().lower() in {"1", "true", "yes"}
