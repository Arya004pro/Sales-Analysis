...
_PARSE_DEGRADED_UNTIL = 0.0
_PARSE_DEGRADED_SECONDS = 120.0


def _build_heuristic_parsed(
    step: ModuleType,
    user_query: str,
    ...
    parsed["_parser_source"] = "heuristic_fallback"
    return parsed
...
async def run_parse_intent(
    ...
    if merged_parsed:
        ...
        now_ts = time.time()
        if now_ts < _PARSE_DEGRADED_UNTIL:
            parsed = _build_heuristic_parsed(
                step,
                user_query,
                mandatory_filters,
                followup_ctx,
            ...
            ctx.logger.warn(
                "Using heuristic parse fallback",
                {
                    "queryId": query_id,
                    "reason": "parse_circuit_breaker_active",
                    "degraded_seconds_left": max(
                        0, int(_PARSE_DEGRADED_UNTIL - now_ts)
                    ),
                },
            )
        else:
            try:
                ...
                if followup_ctx:
                    parsed = step._merge_followup_intent(
                        parsed, user_query, followup_ctx
                    )
                parsed = step._post_process(parsed, user_query, mandatory_filters)
                ...
                ctx.logger.info(
                    "Intent parsed", {"queryId": query_id, "parsed": parsed}
                )
            except Exception as exc:
                ctx.logger.error(
                    "Intent parse failed", {"error": str(exc), "queryId": query_id}
                )
                _PARSE_DEGRADED_UNTIL = time.time() + _PARSE_DEGRADED_SECONDS
                # Heuristic fallback keeps the pipeline working when the LLM parser
                # is rate-limited or returns invalid JSON.
                try:
                    parsed = _build_heuristic_parsed(
                        step,
                        user_query,
                    ...
                    ctx.logger.warn(
                        "Using heuristic parse fallback",
                        {
                            "queryId": query_id,
                            "reason": str(exc),
                            "degraded_for_seconds": int(_PARSE_DEGRADED_SECONDS),
                        },
                    )
                    ...
                    if qs:
                        ...
                        await ctx.state.set(
                            ...
                            query_id,
                            {
                                **qs,
                                "status": "error",
                                "error": f"Parse intent model failed: {exc}; fallback failed: {fallback_exc}",
                                "updatedAt": now_iso,
